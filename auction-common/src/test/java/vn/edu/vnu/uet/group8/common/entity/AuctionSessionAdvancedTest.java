 ...
 * <p>Tập trung vào:
 * <ul>
 *   <li><b>Reconstructor đầy đủ kịch bản</b>: thiếu từng field bắt buộc</li>
 *   <li><b>transitionStatus toàn bộ ma trận</b>: tất cả từ-đến hợp lệ/không hợp lệ</li>
 *   <li><b>isActive() kết hợp soft delete</b>: ACTIVE nhưng bị xóa mềm</li>
 *   <li><b>isInSnipingWindow() các trường hợp biên</b>: window 0, window lớn</li>
 *   <li><b>raiseCurrentPrice liên tiếp</b>: nhiều lần tăng giá trong session</li>
 *   <li><b>incrementBidCount chính xác</b>: tăng từng đơn vị</li>
 *   <li><b>toString()</b>: chứa đủ thông tin debug</li>
 *   <li><b>Builder: startingPrice = 0</b>: edge case giá khởi điểm bằng 0</li>
 * </ul>
 */
...
class AuctionSessionAdvancedTest {
    ...
    private static final Instant FUTURE_2H = Instant.now().plus(2, ChronoUnit.HOURS);
    ...
    // ═══════════════════════════════════════════════════
    // RECONSTRUCTOR - Các trường hợp nâng cao
    // ═══════════════════════════════════════════════════
    ...
    class ReconstructorAdvanced {
        ...
        void reconstructorHighestBidderNull() {
            AuctionSession s = AuctionSession.reconstructor()
                    ...
                    .itemId(1).startingPrice(BigDecimal.TEN).currentPrice(BigDecimal.TEN)
                    .status(SessionStatus.UPCOMING)
                    .startTime(Instant.now()).endTime(FUTURE_1H)
        ...
        void reconstructorThieuItemId() {
            assertThrows(IllegalStateException.class,
                    () -> AuctionSession.reconstructor()
                            .id(1).createdAt(Instant.now()).isDeleted(false)
                            // thiếu itemId
                            .startingPrice(BigDecimal.TEN).currentPrice(BigDecimal.TEN)
                            .status(SessionStatus.UPCOMING)
                            .startTime(Instant.now()).endTime(FUTURE_1H)
                            .bidCount(0).build());
        }
        ...
        void reconstructorThieuCurrentPrice() {
            assertThrows(IllegalStateException.class,
                    () -> AuctionSession.reconstructor()
                            .id(1).createdAt(Instant.now()).isDeleted(false)
                            .itemId(1).startingPrice(BigDecimal.TEN)
                            // thiếu currentPrice
                            .status(SessionStatus.UPCOMING)
                            .startTime(Instant.now()).endTime(FUTURE_1H)
                            .bidCount(0).build());
        ...
        void reconstructorThieuStartTime() {
            assertThrows(IllegalStateException.class,
                    () -> AuctionSession.reconstructor()
                            ...
                            .status(SessionStatus.UPCOMING)
                            // thiếu startTime
                            .endTime(FUTURE_1H).bidCount(0).build());
        }
        ...
        void reconstructorSoftDeleted() {
            AuctionSession s = AuctionSession.reconstructor()
                    ...
                    .startTime(Instant.now()).endTime(FUTURE_1H).bidCount(0).build();
            assertTrue(s.isDeleted());
        }
    }

    // ═══════════════════════════════════════════════════
    // TRANSITION STATUS - Ma trận đầy đủ
    // ═══════════════════════════════════════════════════
    ...
    class TransitionMatrix {
        ...
        void transitionHopLe(String from, String to) throws Exception {
            ...
            boolean result = s.transitionStatus(
                    SessionStatus.valueOf(from),
                    SessionStatus.valueOf(to)
            );
        ...
        void transitionKhongHopLe(String from, String to) throws Exception {
            ...
            assertEquals(SessionStatus.valueOf(from), s.getStatus(),
                    "Status phải giữ nguyên khi transition không hợp lệ");
        }

        @Test
        @DisplayName("transitionStatus với 'from' sai - behavior hiện tại vẫn transition")
    ...
    // ═══════════════════════════════════════════════════
    // isInSnipingWindow() - Các trường hợp biên
    // ═══════════════════════════════════════════════════
    ...
    class SnipingWindowEdgeCases {
        ...
        void windowZeroPhut() {
            AuctionSession s = new AuctionSession.Builder(1, BigDecimal.TEN,
                    Instant.now(), FUTURE_1H).build();
            // window 0 phút: Instant.now().isAfter(endTime - 0) = isAfter(endTime) = false
        ...
        void windowNhoHonThoiGianConLai() {
            ...
            AuctionSession s = new AuctionSession.Builder(1, BigDecimal.TEN,
                    Instant.now(), end30s).build();
            // end - 1 phút = now - 30s → now không isAfter(now - 30s)... thực ra là IS after
            // end - 60s = now - 30s → now > (now - 30s) → TRUE
            assertTrue(s.isInSnipingWindow(1),
                    "endTime = now+30s, window 1 phút → now đang trong window");
        }
        ...
        void windowNhieuHonThoiGianConLai() {
            ...
            assertFalse(s.isInSnipingWindow(5),
                    "Còn 1 giờ nhưng window chỉ 5 phút → chưa trong window");
        }
        ...
        void windowLonHonThoiGianConLai() {
            AuctionSession s = new AuctionSession.Builder(1, BigDecimal.TEN,
                    Instant.now(), FUTURE_1H).build();
            // window = 120 phút > 60 phút còn lại → đang trong window
            assertTrue(s.isInSnipingWindow(120));
        }
    }

    // ═══════════════════════════════════════════════════
    // raiseCurrentPrice() LIÊN TIẾP
    // ═══════════════════════════════════════════════════
    ...
    class RaisePriceSequential {
        ...
        void tangGiaRoiThuThapHon() throws Exception {
            ...
            assertThrows(IllegalStateException.class,
                    () -> s.raiseCurrentPrice(new BigDecimal("150000")));
            // Giá vẫn là 200k
    ...
    class BuilderEdgeCases {
        ...
        void startingPriceZero() {
            assertDoesNotThrow(() -> new AuctionSession.Builder(
                    1, BigDecimal.ZERO, Instant.now(), FUTURE_1H).build());
        }
        ...
        void bidCountNullDefault() {
            AuctionSession s = new AuctionSession.Builder(
                    1, BigDecimal.TEN, Instant.now(), FUTURE_1H)
                    .bidCount(null)
                    .build();
            assertEquals(0, s.getBidCount());
        }
        ...
        void currentPriceEqualStartingPrice() {
            ...
            AuctionSession s = new AuctionSession.Builder(1, startPrice, Instant.now(), FUTURE_1H)
                    .build();
            assertEquals(0, s.getCurrentPrice().compareTo(startPrice),
    ...
    void toStringChuaThongTin() {
        AuctionSession s = AuctionSession.reconstructor()
                .id(42).createdAt(Instant.now()).isDeleted(false)
                .itemId(7)
