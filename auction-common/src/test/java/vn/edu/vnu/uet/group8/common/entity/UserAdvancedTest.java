 ...
 *
 * <p>Tập trung vào các kịch bản nghiệp vụ CHƯA được cover trong test cơ bản:
 * <ul>
 *   <li><b>recordLogin()</b>: cập nhật lastLogin sau khi đăng nhập</li>
 *   <li><b>addRole / removeRole logic</b>: không xóa hết role, không trùng</li>
 *   <li><b>isActive với soft delete</b>: ACTIVE nhưng bị xóa → không active</li>
 *   <li><b>getDisplayRole() đầy đủ kịch bản</b>: BIDDER only, SELLER only, cả hai</li>
 *   <li><b>setEncryptedPassword()</b>: đổi mật khẩu có kiểm soát</li>
 *   <li><b>setStatus() cho UserMember</b>: ACTIVE → SUSPENDED → BANNED → ACTIVE</li>
 *   <li><b>UserMember Reconstructor</b>: đầy đủ field và thiếu field bắt buộc</li>
 *   <li><b>UserMember setAddress/setAvatarUrl</b>: nullable fields</li>
 *   <li><b>Sealed class</b>: chỉ UserMember và UserAdmin kế thừa User</li>
 *   <li><b>Đa hình (polymorphism)</b>: User reference trỏ đến UserMember/UserAdmin</li>
 * </ul>
 */
...
class UserAdvancedTest {
    ...
    private UserAdmin createModerator() {
        return new UserAdmin.Builder("mod1", "mod@example.com", "$2a$hash",
                AdminLevel.MODERATOR).build();
    }

    private UserAdmin createSuperAdmin() {
        return new UserAdmin.Builder("super", "super@example.com", "$2a$hash",
    ...
    class RecordLogin {
        ...
        void recordLoginNhieuLan() throws InterruptedException {
            ...
            assertTrue(secondLogin.isAfter(firstLogin),
                    "Login lần 2 phải có lastLogin mới hơn lần 1");
        }
    ...
    class RoleManagementAdvanced {
        ...
        @ParameterizedTest
        @EnumSource(value = UserRole.class, names = {"BIDDER", "SELLER"})
    ...
    // ═══════════════════════════════════════════════════
    // isActive() - Nâng cao với soft delete
    // ═══════════════════════════════════════════════════
    ...
    class IsActiveAdvanced {
        ...
        void activeButSoftDeleted() {
            ...
            assertFalse(m.isActive(),
                    "User bị xóa mềm không được tính là active dù status = ACTIVE");
        }
    ...
    // ═══════════════════════════════════════════════════
    // getDisplayRole() - Đầy đủ kịch bản
    // ═══════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════
    // UserMember Reconstructor - Nâng cao
    // ═══════════════════════════════════════════════════
        ...
        void reconstructorThieuBalance() {
            assertThrows(IllegalStateException.class,
                    () -> UserMember.reconstructor()
                            ...
                            .username("x").email("x@x.com").encryptedPassword("$h")
                            .status(UserStatus.ACTIVE).roles(EnumSet.of(UserRole.BIDDER))
                            // thiếu balance và phone
                            .build());
        }
    ...
    // ═══════════════════════════════════════════════════
    // UserMember setAddress & setAvatarUrl
    // ═══════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════
    // ĐA HÌNH (POLYMORPHISM) - Sealed class
    // ═══════════════════════════════════════════════════
    ...
    class Normalization {
        ...
        void usernameNormalize() {
            UserMember m = UserMember.builder("  ALICE_DEV  ", "a@e.com", "$hash")
                    .phone("0901234567")
                    .build();
        ...
        void emailNormalize() {
            UserMember m = UserMember.builder("alice", "  ALICE@EXAMPLE.COM  ", "$hash")
                    .phone("0901234567")
                    .build();
        ...
        void usernameKhongBiThayDoiNeudaLower() {
            UserMember m = UserMember.builder("alice_123", "a@e.com", "$hash")
                    .phone("0901234567")
                    .build();
    ...
    class BalanceTest {
        ...
        void balanceMacDinhZero() {
            UserMember m = UserMember.builder("u", "u@e.com", "$h")
                    .phone("0901234567")
                    .build();
        ...
        void balanceAmBiReject() {
            UserMember m = UserMember.builder("u", "u@e.com", "$h")
                    ...
                    .build();
            assertEquals(0, m.getBalance().compareTo(BigDecimal.ZERO),
                    "Balance âm phải bị reject, giữ mặc định ZERO");
        }
