...
class AuctionEventSubscriberTest {
  ...
  void registerTo() {
    ...
    verify(eventBus).subscribe(eq(BidPlacedEvent.class), any());
    verify(eventBus).subscribe(eq(AuctionEndedEvent.class), any());
  }
  ...
  class OnBidPlacedTest {
    ...
    private AuctionEventBus.EventListener<BidPlacedEvent> getBidHandler() {
      ...
      verify(eventBus).subscribe(eq(BidPlacedEvent.class), captor.capture());
    ...
    void broadcastPriceUpdate() throws Exception {
      AuctionEventBus.EventListener<BidPlacedEvent> handler = getBidHandler();

      handler.onEvent(buildEvent(null));

      ArgumentCaptor<ServerResponse> respCaptor = ArgumentCaptor.forClass(ServerResponse.class);

      verify(channel).broadcast(respCaptor.capture());

      assertEquals("PRICE_UPDATE", respCaptor.getValue().getMessage());
    }
    ...
    void outbidNotification() throws Exception {
      ...
      verify(channel).sendToUser(eq(7), any(ServerResponse.class));
    }
    ...
    void sameBidderNoOutbid() throws Exception {
      ...
      verify(channel, never()).sendToUser(anyInt(), any());
    }
    ...
    void notifServiceErrorSilent() throws Exception {
      ...
      assertDoesNotThrow(() -> handler.onEvent(buildEvent(7)));
  ...
  class OnAuctionEndedTest {
    ...
    @SuppressWarnings("unchecked")
    private AuctionEventBus.EventListener<AuctionEndedEvent> getEndedHandler() {
      subscriber.registerTo(eventBus);

      ArgumentCaptor<AuctionEventBus.EventListener<AuctionEndedEvent>> captor =
          ArgumentCaptor.forClass((Class) AuctionEventBus.EventListener.class);

      verify(eventBus).subscribe(eq(AuctionEndedEvent.class), captor.capture());
    ...
    void soldBroadcast() throws Exception {
      ...
      ArgumentCaptor<ServerResponse> captor = ArgumentCaptor.forClass(ServerResponse.class);

      verify(channel).broadcast(captor.capture());

      assertTrue(captor.getValue().getMessage().contains("25000000"));
    }
    ...
    void noBidBroadcast() throws Exception {
      ...
      assertTrue(captor.getValue().getMessage().contains("không có người đặt giá"));
    }
    ...
    void notifServiceErrorSilent() throws Exception {
      ...
      assertDoesNotThrow(() -> handler.onEvent(buildSoldEvent(7)));
