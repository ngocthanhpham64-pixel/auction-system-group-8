 ...
 *
 * <p>Chỉ test {@code deleteItem()} vì createItem + updateItem
 * dùng FileUtil.saveBase64Images (static, khó mock).
 */
...
class ItemWriteServiceTest {
    ...
    private UserAdmin taoAdmin(int id, AdminLevel level) {
        UserAdmin a = new UserAdmin.Builder("admin" + id, "a" + id + "@e.com", "hash", level).build();
        a.assignId(id);
        return a;
    }
