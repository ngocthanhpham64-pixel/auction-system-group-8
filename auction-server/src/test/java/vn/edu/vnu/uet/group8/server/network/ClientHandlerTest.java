...
class ClientHandlerTest {
    ...
    void getUserIdNullBeforeLogin() {
        ClientHandler handler =
                new ClientHandler(socket, dispatcher, broadcastChannel);
    ...
    void runIOExceptionOnSetup() throws Exception {
        ...
        when(socket.getInputStream())
                .thenThrow(new IOException("connect failed"));
    ...
    class RunRequestTest {
        ...
        void validRequest() throws Exception {
            ...
            String json =
                    "{\"action\":\"HEARTBEAT\",\"requestId\":\"r1\"}";

            InputStream is =
                    buildInputStream(encodeMessage(json));
        ...
        void loginSuccessRegistersUser() throws Exception {
            ...
            UserMember member =
                    UserMember.builder("user01", "u@e.com", "pw").build();

            UserSummaryDTO summary = UserSummaryDTO.from(member);

            LoginResultDTO loginResult =
                    new LoginResultDTO(summary, "token-xyz");

            ServerResponse loginResp =
                    ServerResponse.reply("LOGIN", "req-login")
                            .success(true)
                            .message("ok")
                            .data(loginResult)
                            .build();
            ...
            verify(broadcastChannel)
                    .registerUser(eq(loginResult.getUserId()), eq(handler));
        ...
        void loginResponseDataWrongType() throws Exception {
            ...
            verify(broadcastChannel, never())
                    .registerUser(anyInt(), any());
        ...
        void logoutAfterLogin() throws Exception {
            ...
            ServerResponse logoutResp =
                    ServerResponse.reply("LOGOUT", "req-logout")
                            .success(true)
                            .message("ok")
                            .build();
            ...
            verify(broadcastChannel)
                    .unregisterUser(anyInt(), eq(handler));
