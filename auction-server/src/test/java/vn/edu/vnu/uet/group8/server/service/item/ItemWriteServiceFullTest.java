...
class ItemWriteServiceFullTest {
  ...
  class CreateItemHappyPath {
    ...
    void taoItemDraftThanhCong() throws SQLException {
      ...
      try (MockedStatic<FileUtil> mf = mockStatic(FileUtil.class)) {
        ...
        Item result = service.createItem(5, "MacBook Pro", "Mô tả laptop",
            ItemCategory.ELECTRONICS, ItemCondition.NEW,
            List.of("base64data=="), null, null, null);
    ...
    void taoItemPublishNgay() throws SQLException {
      ...
      try (MockedStatic<FileUtil> mf = mockStatic(FileUtil.class)) {
        mf.when(() -> FileUtil.saveBase64Images(anyList())).thenReturn(FAKE_URLS);
        doAnswer(inv -> {
          ((Item) inv.getArgument(0)).assignId(101);
          return null;
        }).when(itemDAO).insert(any(Item.class));

        Item result = service.createItem(5, "iPhone 17", "Điện thoại mới",
            ItemCategory.ELECTRONICS, ItemCondition.NEW,
            List.of(), new BigDecimal("5000000"), 24, null);
    ...
    void sellerDaCORoleKhongAddLai() throws SQLException {
      UserMember seller = activeSeller(5);
      when(userDAO.findById(5)).thenReturn(Optional.of(seller));
      when(itemDAO.countBySellerAndStatus(5, ItemStatus.LISTED)).thenReturn(0);
      ...
      try (MockedStatic<FileUtil> mf = mockStatic(FileUtil.class)) {
        mf.when(() -> FileUtil.saveBase64Images(anyList())).thenReturn(FAKE_URLS);
        doAnswer(inv -> {
          ((Item) inv.getArgument(0)).assignId(102);
          return null;
        }).when(itemDAO).insert(any(Item.class));
    ...
    void imageUrlsNull() throws SQLException {
      ...
      try (MockedStatic<FileUtil> mf = mockStatic(FileUtil.class)) {
        ...
        doAnswer(inv -> {
          ((Item) inv.getArgument(0)).assignId(103);
          return null;
        }).when(itemDAO).insert(any(Item.class));
    ...
    void imageUrlsRong() throws SQLException {
      ...
      try (MockedStatic<FileUtil> mf = mockStatic(FileUtil.class)) {
        ...
        doAnswer(inv -> {
          ((Item) inv.getArgument(0)).assignId(104);
          return null;
        }).when(itemDAO).insert(any(Item.class));
    ...
    void durationHoursZeroKhongPublish() throws SQLException {
      ...
      try (MockedStatic<FileUtil> mf = mockStatic(FileUtil.class)) {
        mf.when(() -> FileUtil.saveBase64Images(anyList())).thenReturn(FAKE_URLS);
        doAnswer(inv -> {
          ((Item) inv.getArgument(0)).assignId(105);
          return null;
        }).when(itemDAO).insert(any(Item.class));

        Item result = service.createItem(5, "Laptop", "Desc",
            ItemCategory.ELECTRONICS, ItemCondition.NEW,
            List.of(), new BigDecimal("1000000"), 0, null);
    ...
    void durationHoursAmKhongPublish() throws SQLException {
      ...
      try (MockedStatic<FileUtil> mf = mockStatic(FileUtil.class)) {
        mf.when(() -> FileUtil.saveBase64Images(anyList())).thenReturn(FAKE_URLS);
        doAnswer(inv -> {
          ((Item) inv.getArgument(0)).assignId(106);
          return null;
        }).when(itemDAO).insert(any(Item.class));

        Item result = service.createItem(5, "Laptop", "Desc",
            ItemCategory.ELECTRONICS, ItemCondition.NEW,
            List.of(), new BigDecimal("1000000"), -5, null);
    ...
    void taoItemNhieuCategory(String categoryName) throws SQLException {
      ...
      try (MockedStatic<FileUtil> mf = mockStatic(FileUtil.class)) {
        mf.when(() -> FileUtil.saveBase64Images(anyList())).thenReturn(FAKE_URLS);
        doAnswer(inv -> {
          ((Item) inv.getArgument(0)).assignId(200);
          return null;
        }).when(itemDAO).insert(any(Item.class));

        assertDoesNotThrow(() ->
            service.createItem(5, "Sản phẩm " + cat, "Desc", cat,
                ItemCondition.NEW, List.of(), null, null, null));
      }
    }
    ...
    void specValidatorDuocGoiDung() throws SQLException {
      ...
      try (MockedStatic<FileUtil> mf = mockStatic(FileUtil.class)) {
        mf.when(() -> FileUtil.saveBase64Images(anyList())).thenReturn(FAKE_URLS);
        doAnswer(inv -> {
          ((Item) inv.getArgument(0)).assignId(99);
          return null;
        }).when(itemDAO).insert(any(Item.class));
    ...
    void sessionEndTimeDung() throws SQLException {
      ...
      long timeBefore = Instant.now().minusSeconds(1).getEpochSecond();

      try (MockedStatic<FileUtil> mf = mockStatic(FileUtil.class)) {
        mf.when(() -> FileUtil.saveBase64Images(anyList())).thenReturn(FAKE_URLS);
        doAnswer(inv -> {
          ((Item) inv.getArgument(0)).assignId(110);
          return null;
        }).when(itemDAO).insert(any(Item.class));

        service.createItem(5, "Watch", "Desc",
            ItemCategory.JEWELRY, ItemCondition.NEW,
            List.of(), new BigDecimal("2000000"), 3, null);

        ArgumentCaptor<AuctionSession> cap = ArgumentCaptor.forClass(AuctionSession.class);
        verify(sessionDAO).insert(cap.capture());

        AuctionSession s = cap.getValue();
        long timeAfter = Instant.now().plusSeconds(1).getEpochSecond();

        long MINUTES_3_SECS = 3 * 60;
        assertTrue(s.getEndTime().getEpochSecond() >= (timeBefore + MINUTES_3_SECS));
        assertTrue(s.getEndTime().getEpochSecond() <= (timeAfter + MINUTES_3_SECS));
      }
    }
  }
  ...
  class UpdateItemHappyPath {
    ...
    void capNhatTitleHopLe() throws SQLException {
      ...
      try (MockedStatic<FileUtil> mf = mockStatic(FileUtil.class)) {
        ...
        service.updateItem(5, 10, "Samsung Galaxy S25", null, null, null);
    ...
    void capNhatDescription() throws SQLException {
      ...
      try (MockedStatic<FileUtil> mf = mockStatic(FileUtil.class)) {
        ...
        service.updateItem(5, 10, null, "Mô tả mới rất chi tiết", null, null);
    ...
    void capNhatDongThoi() throws SQLException {
      ...
      try (MockedStatic<FileUtil> mf = mockStatic(FileUtil.class)) {
        ...
        assertEquals(ItemCondition.LIKENEW, item.getCondition());
    ...
    void updateGoiDung() throws SQLException {
      ...
      try (MockedStatic<FileUtil> mf = mockStatic(FileUtil.class)) {
        ...
        service.updateItem(5, 10, "New Title", "New Desc", ItemCondition.USED, null);
  ...
  class DeleteItemExtra {
    ...
    void nhieuSessionCoActive() throws SQLException {
      ...
      AuctionException ex = assertThrows(AuctionException.class, () -> service.deleteItem(5, 10));
      assertTrue(ex.getMessage().contains("Hủy phiên"), "Thông báo lỗi phải nhắc hủy phiên trước");
    ...
    void adminModeratorXoa() throws SQLException {
      ...
      UserAdmin mod = new UserAdmin.Builder(
          "mod", "mod@mail.com", "$2a$hash", AdminLevel.MODERATOR).build();
      mod.assignId(99);
