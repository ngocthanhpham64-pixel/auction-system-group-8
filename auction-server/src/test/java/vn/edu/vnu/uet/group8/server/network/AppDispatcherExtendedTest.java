...
class AppDispatcherExtendedTest {
    ...
    class AnonymousActionsTest {
        ...
        @Test @DisplayName("AUTH_REQUEST_OTP → authController.handleRequestOtp()")
        void authRequestOtp() throws Exception {
            when(authController.handleRequestOtp(any(), any())).thenReturn(OK);
            ServerResponse r = dispatcher.dispatch(req("AUTH_REQUEST_OTP"));
            assertTrue(r.isSuccess());
            verify(authController).handleRequestOtp(any(), any());
        }

        @Test @DisplayName("auth_request_otp lowercase → normalize đúng")
        void authRequestOtpLowercase() throws Exception {
            when(authController.handleRequestOtp(any(), any())).thenReturn(OK);
            dispatcher.dispatch(req("auth_request_otp"));
            verify(authController).handleRequestOtp(any(), any());
        }

        @Test @DisplayName("AUTH_RESET_PASSWORD → authController.handleResetPassword()")
        void authResetPassword() throws Exception {
            when(authController.handleResetPassword(any(), any())).thenReturn(OK);
            ServerResponse r = dispatcher.dispatch(req("AUTH_RESET_PASSWORD"));
            assertTrue(r.isSuccess());
    ...
    class UserActionsTest {
        ...
        @Test @DisplayName("USER_PURCHASE_HISTORY → handleGetPurchaseHistory()")
    ...
    class TokenInvalidTest {
        ...
        void tokenInvalidBlock(String action) {
            ServerResponse r = dispatcher.dispatch(req(action));
    ...
    class ControllerExceptionTest {
        ...
        @Test @DisplayName("ITEM_CREATE ném RuntimeException → trả error response")
        void itemCreateThrow() throws Exception {
            ...
            ServerResponse r = dispatcher.dispatch(req("ITEM_CREATE"));
            assertFalse(r.isSuccess());
            assertTrue(r.getMessage().contains("Lỗi máy chủ") || r.getMessage().contains("DB sập"));
        }

        @Test @DisplayName("ADMIN_DASHBOARD ném Exception → trả error response")
        void adminDashboardThrow() throws Exception {
            ...
            ServerResponse r = dispatcher.dispatch(req("ADMIN_DASHBOARD"));
            assertFalse(r.isSuccess());
        }

        @Test @DisplayName("BID_PLACE ném Exception → action trong response = BID_PLACE")
        void bidPlaceThrowPreservesAction() throws Exception {
            when(bidController.handlePlaceBid(any(), any(), anyInt()))
                    .thenThrow(new RuntimeException("timeout"));
            ServerResponse r = dispatcher.dispatch(req("BID_PLACE"));
            assertFalse(r.isSuccess());
    ...
    void missingActionField() {
        ...
        assertThrows(Exception.class, () -> dispatcher.dispatch(r));
    }
}