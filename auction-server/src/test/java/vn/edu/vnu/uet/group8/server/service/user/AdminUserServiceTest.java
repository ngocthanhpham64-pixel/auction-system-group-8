 ...
 *
 * <p>Test các method không phụ thuộc DatabaseConnection static:
 * updateUserStatus, cancelAuction. Các method khác (getDashboardStats,
 * getUsers, getAuctions) dùng DatabaseConnection.getInstance() static → khó mock.
 */
...
class AdminUserServiceTest {
    ...
    private UserMember taoMember(int id, UserStatus status) {
        ...
        m.assignId(id);
        if (status != UserStatus.ACTIVE) m.setStatus(status);
        return m;
    }

    private UserAdmin taoAdmin(int id, AdminLevel level) {
        UserAdmin a = new UserAdmin.Builder("admin" + id, "adm" + id + "@e.com",
                "hash", level).build();
        a.assignId(id);
        return a;
    }
