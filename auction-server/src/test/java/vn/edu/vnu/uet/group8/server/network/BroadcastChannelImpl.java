...
class BroadcastChannelImplTest {
    ...
    class AddClientTest {
        ...
        @Test @DisplayName("addClient null → count không tăng")
        void nullIgnored() {
            impl.addClient(null);
            assertEquals(0, impl.getConnectedClientCount());
        }

        @Test @DisplayName("addClient 1 client → count = 1")
        void oneClient() {
            impl.addClient(h1);
            assertEquals(1, impl.getConnectedClientCount());
        }

        @Test @DisplayName("addClient 3 clients → count = 3")
        void threeClients() {
            impl.addClient(h1);
            impl.addClient(h2);
            impl.addClient(h3);
        ...
        @Test @DisplayName("addClient cùng handler 2 lần → count = 1 (Set)")
    ...
    class RemoveClientTest {
        ...
        @Test @DisplayName("removeClient null → không throw")
        void nullIgnored() {
            impl.addClient(h1);
        ...
        @Test @DisplayName("removeClient handler có trong set → count giảm")
        void removeExisting() {
            impl.addClient(h1);
            impl.addClient(h2);
        ...
        @Test @DisplayName("removeClient handler không có → count không đổi")
        ...
        @Test @DisplayName("removeClient có userId → unregister khỏi userSessions")
        ...
        @Test @DisplayName("removeClient userId null → không throw")
        ...
        @Test @DisplayName("removeClient với userId không có trong userSessions → không throw")
    ...
    class BroadcastTest {
        ...
        @Test @DisplayName("broadcast khi không có client → không throw")
        void noClients() {
            assertDoesNotThrow(() -> impl.broadcast(resp("T")));
        }

        @Test @DisplayName("broadcast → gửi tới tất cả 3 client")
        void allClients() {
            impl.addClient(h1);
            impl.addClient(h2);
            impl.addClient(h3);
        ...
        @Test @DisplayName("broadcast một client lỗi → bỏ qua, gửi tiếp cho các client khác")
        void oneClientFails() {
            impl.addClient(h1);
            impl.addClient(h2);
        ...
        @Test @DisplayName("broadcast khi tất cả client lỗi → không throw")
        void allClientsFail() {
            impl.addClient(h1);
            impl.addClient(h2);
        ...
        @Test @DisplayName("broadcast → getConnectedClientCount() dùng để log")
        void connectedCount() {
            impl.addClient(h1);
            impl.addClient(h2);
    ...
    class RegisterUserTest {
        ...
        @Test @DisplayName("registerUser null handler → bỏ qua")
        void registerNull() {
            assertDoesNotThrow(() -> impl.registerUser(1, null));
        }

        @Test @DisplayName("unregisterUser null handler → bỏ qua")
        void unregisterNull() {
            assertDoesNotThrow(() -> impl.unregisterUser(1, null));
        }

        @Test @DisplayName("registerUser 2 handler cùng userId → cả 2 nhận tin")
        ...
        @Test @DisplayName("unregisterUser → handler đó không nhận tin nữa")
        ...
        @Test @DisplayName("unregisterUser khi userId không có trong map → không throw")
    ...
    class SendToUserTest {
        ...
        @Test @DisplayName("user không online → không gửi, không throw")
        void userOffline() {
            assertDoesNotThrow(() -> impl.sendToUser(999, resp("T")));
            verify(h1, never()).send(any());
        }

        @Test @DisplayName("user có 1 session → gửi đúng")
        void oneSession() {
        ...
        @Test @DisplayName("user có nhiều session → gửi tất cả")
        ...
        @Test @DisplayName("session lỗi khi send → loại khỏi danh sách, không throw")
        ...
        @Test @DisplayName("sendToUser sau khi tất cả session bị unregister → không gửi")
    ...
    // ─────────────────────────────────────────────────────────────
    @Test @DisplayName("getConnectedClientCount = 0 ban đầu")
    void initialCount() {
        assertEquals(0, impl.getConnectedClientCount());
    }

    @Test @DisplayName("getConnectedClientCount cập nhật đúng sau add/remove")
