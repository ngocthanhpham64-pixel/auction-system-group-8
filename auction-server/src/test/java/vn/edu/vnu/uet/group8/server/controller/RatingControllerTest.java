...
class RatingControllerTest {
    ...
    private JsonObject reqWithPayload(int sellerId, String username, int score, String comment) {
    ...
    class RateSellerTest {
        ...
        void payloadAtRoot() throws SQLException {
            ...
            assertTrue(resp.isSuccess());
            verify(ratingService).rateSeller(1, "seller_x", 3, 3, null);
        }
        ...
        void validationError() throws SQLException {
            doThrow(new ValidationException("Không thể tự đánh giá"))
                    .when(ratingService).rateSeller(anyInt(), anyString(), anyInt(), anyInt(), any());
            JsonObject req = reqWithPayload(1, "self", 5, null); // raterId == sellerId
        ...
        void sqlException() throws SQLException {
            doThrow(new SQLException("DB lỗi"))
                    .when(ratingService).rateSeller(anyInt(), anyString(), anyInt(), anyInt(), any());
            JsonObject req = reqWithPayload(2, "s", 4, "ok");
