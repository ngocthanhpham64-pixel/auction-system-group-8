...
public final class UserService{
        ...
        if(response.isSuccess()){
            ...
            if(user != null){
                ...
                if (user.getAvatarUrl() != null) {
                    ClientModel.getInstance().setAvatarUrl(user.getAvatarUrl());
                } else {
                    ClientModel.getInstance().setAvatarUrl("");
                }
                ...
                // Cập nhật số dư nếu có
                if (user.getBalance() != null) {
                    ClientModel.getInstance().updateBalance(user.getBalance());
                    LOGGER.info("Updated ClientModel balance to: " + user.getBalance());
                }
                LOGGER.fine("Profile loaded: " + user.getUsername());
            } else{
                LOGGER.warning("loadProfile received null data despite success response");
                ClientModel.getInstance().updateBalance(BigDecimal.ZERO); // Ensure balance is not null
            }
    ...
    AuctionClient.getInstance().sendAuthenticatedRequest(
            ...
            response -> {
                if (response.isSuccess()) {
                    ...
                    // Force reload profile để đảm bảo đồng bộ mới nhất
                    loadProfile(null);
  ...
  /**
   * Lấy danh sách sản phẩm đã mua (thắng thầu).
   */
  public static void loadPurchaseHistory(Consumer<List<AuctionItemDTO>> onResult) {
    AuctionClient.getInstance().sendAuthenticatedRequest(
        ActionType.USER_PURCHASE_HISTORY, null,
        response -> {
            if (response.isSuccess()) {
                List<AuctionItemDTO> list = GsonUtil.toList(response.getData(), AuctionItemDTO.class);
                if (onResult != null) onResult.accept(list != null ? list : List.of());
            } else {
                if (onResult != null) onResult.accept(List.of());
            }
  ...
  public static void updateProfile(String fullname, String phone, String address, String avatarBase64, Consumer<Boolean> onResult) {
      ...
      AuctionClient.getInstance().sendAuthenticatedRequest(
          ...
          response -> {
              if (response.isSuccess()) {
                  loadProfile(null); // Tải lại thông tin để cập nhật UI
              } else {
