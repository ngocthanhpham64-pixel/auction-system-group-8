...
public final class AuthService {
  ...
  public static void login(String email, String password, Runnable onSuccess, Consumer<String> onFailure){
    ...
    AuctionClient.getInstance().sendRequest(request,response -> {
        ...
        String role = loginResult.user().isAdmin() ? loginResult.user().getDisplayRole() : "MEMBER";
  ...
  public static void requestOtp(String email, Consumer<String> onSuccess, Consumer<String> onFailure) {
    ...
    ServerRequest<Map<String, String>> req = ServerRequest.<Map<String, String>>builder(ActionType.AUTH_REQUEST_OTP)
    ...
    AuctionClient.getInstance().sendRequest(req, response -> {
      try {
        if (response.isSuccess()) {
          // Ép kiểu an toàn, nếu data có vấn đề sẽ bị catch ngay lập tức
          String otp = GsonUtil.toObject(response.getData(), String.class);
          onSuccess.accept(otp); 
        } else {
          onFailure.accept(response.getMessage());
        }
      } catch (Exception e) {
        // Bắt mọi lỗi xảy ra khi xử lý response để UI không bị treo
        onFailure.accept("Lỗi xử lý dữ liệu từ server: " + e.getMessage());
      }
    });
  }
  ...
  public static void resetPassword(String email, String otp, String newPassword, Runnable onSuccess, Consumer<String> onFailure) {
    ...
    ServerRequest<Map<String, String>> req = ServerRequest.<Map<String, String>>builder(ActionType.AUTH_RESET_PASSWORD)
